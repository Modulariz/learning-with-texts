...
if (isset($_REQUEST['new'])) {
	...
	<td class="td1"><input class="notempty setfocus noblanksnocomma checkoutsidebmp" type="text" name="T2Text" data_info="Tag" value="" maxlength="20" size="20" /> <img src="<?php echo $_ENV['BASE_URL']; ?>/icn/status-busy.png" title="Field must not be empty" alt="Field must not be empty" /></td>
	</tr>
	<tr>
...
?>
		...
		<tr>
		<td class="td1 right">Tag:</td>
		<td class="td1"><input data_info="Tag" class="notempty setfocus noblanksnocomma checkoutsidebmp" type="text" name="T2Text" value="<?php echo tohtml($record['T2Text']); ?>" maxlength="20" size="20" /> <img src="<?php echo $_ENV['BASE_URL']; ?>/icn/status-busy.png" title="Field must not be empty" alt="Field must not be empty" /></td>
		</tr>
		<tr>
		<td class="td1 right">Comment:</td>
...
?>
<p><a href="<?php echo $_ENV['BASE_URL'].$_SERVER['PHP_SELF']; ?>?new=1"><img src="<?php echo $_ENV['BASE_URL']; ?>/icn/plus-button.png" title="New" alt="New" /> New Text Tag ...</a></p>
...
while ($record = mysqli_fetch_assoc($res)) {
	...
	echo '<td class="td1 center" nowrap="nowrap">&nbsp;<a href="' . $_ENV['BASE_URL'].$_SERVER['PHP_SELF'] . '?chg=' . $record['T2ID'] . '"><img src="'.$_ENV['BASE_URL'].'//icn/document--pencil.png" title="Edit" alt="Edit" /></a>&nbsp; <a class="confirmdelete" href="' . $_ENV['BASE_URL'].$_SERVER['PHP_SELF'] . '?del=' . $record['T2ID'] . '"><img src="'.$_ENV['BASE_URL'].'/icn/minus-button.png" title="Delete" alt="Delete" /></a>&nbsp;</td>';
	echo '<td class="td1 center">' . tohtml($record['T2Text']) . '</td>';
	echo '<td class="td1 center">' . tohtml($record['T2Comment']) . '</td>';
	echo '<td class="td1 center">' . ($c > 0 ? '<a href="'.$_ENV['BASE_URL'].'/edit_texts.php?page=1&amp;query=&amp;tag12=0&amp;tag2=&amp;tag1=' . $record['T2ID'] . '">' . $c . '</a>' : '0' ) . '</td>';
	echo '<td class="td1 center">' . ($ca > 0 ? '<a href="'.$_ENV['BASE_URL'].'/edit_archivedtexts.php?page=1&amp;query=&amp;tag12=0&amp;tag2=&amp;tag1=' . $record['T2ID'] . '">' . $ca . '</a>' : '0' ) . '</td>';
	echo '</tr>';
}
mysqli_free_result($res);
